...
#include "../io_interfaces/digital_input.h"
 ...
 */
class MotorCommand: public WriteInterface
{
public:
 ...
 */
class ES030Output: public WriteInterface
{
public:
  /**
  ...
  bool write(double value)
  {
    ...
    // Debug info
    ROS_DEBUG_THROTTLE(1.0, "%s: Writing %.2f [V or A], %f [mV or mA], %i bits", name_.c_str(), value, m_value, i_value);
...
//    outputs_[16] = std::make_shared<DigitalOutput<8> >("Reserved4", line_output_ptr, 7);  // Do not expose?!
    outputs_[17] = std::make_shared<AnalogOutput<int16> >("AnalogOut1", &output_struct_ptr->analog_out_1, 12, 0.0, 10.0, -2048);
    outputs_[18] = std::make_shared<AnalogOutput<int16> >("AnalogOut2", &output_struct_ptr->analog_out_2, 12, 0.0, 10.0, -2048);
