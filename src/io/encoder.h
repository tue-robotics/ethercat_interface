...
public:
  /**
   * @brief Encoder Constructor. Handles overflow, assuming raw data is a 16-bit size_teger
   ...
   */
  Encoder(std::string name, T* data_ptr) : data_ptr_(data_ptr), Input(name)
  {
  ...
  double read()
  {
    ...
    ROS_DEBUG_THROTTLE(1.0, "Encoder read value: %d", new_value);
